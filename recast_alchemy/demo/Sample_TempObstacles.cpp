...
dtObstacleRef Sample_TempObstacles::addTempObstacle(const float* pos)
{
	if (!m_tileCache)
		return 0;
	float p[3];
...
dtObstacleRef Sample_TempObstacles::addTempObstacle(const float* pos, const float radius, const float height )
{
	if (!m_tileCache)
		return 0;
	float p[3];
...
void Sample_TempObstacles::removeTempObstacle(dtObstacleRef ref)
{
	if (!m_tileCache)
		return;
...
bool Sample_TempObstacles::handleBuild()
{
	...
	char* tsstr;
	sprintf(tsstr, "tilesize %f", m_tileSize);
